...
<<<<<<< HEAD
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/ctms/api/tournaments?id=${id}`, { withCredentials: true })
      .then(res => {
        const data = res.data;
        setFormData({
          ...data,
...
<<<<<<< HEAD
  ...
  if (loading) {
    return <div className="tlc-page">Loading...</div>;
  }
    ...
    <div className="cw-page">
      <div className="cw-container">
        ...
        <div className="cw-page-header">
          <div>
            <h1>Cập nhật giải đấu</h1>
            <p>Chỉnh sửa thông tin giải cờ vua</p>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="cw-review-notice">
        <ShieldCheck size={20} />
        <div>
          <strong>Xem lại thay đổi</strong>
          Hãy đảm bảo mọi thông tin chính xác trước khi lưu.
        </div>
      </div>
          ...
          <div className="cw-review-card">
            ...
            <div className="cw-review-card-body">
              ...
              <div className="cw-review-row">
                ...
                <span className="value">
                  {Number(data.entryFee) > 0 ? `$${data.entryFee}` : "Miễn phí"}
                </span>
              </div>
              <div className="cw-review-row">
