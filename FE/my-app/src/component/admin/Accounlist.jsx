...
/* ✅ nhận prop onViewAccount */
export const AccountListScreen = ({ onViewAccount }) => {
  ...
  const activeCount = useMemo(() => users.filter((u) => u.isActive === true).length, [users]);
  const lockedCount = total - activeCount;

  // ✅ Toggle trạng thái: update DB bằng BE, sau đó reload list từ DB
  const toggleActive = async (userId, currentActive, usernameOrEmail) => {
    ...
    const okConfirm = window.confirm(`Bạn có chắc muốn ${nextLabel} tài khoản${who} không?`);
    if (!okConfirm) return;
    ...
    try {
      const url = `${API_BASE}/api/admin/users/${userId}/toggle-active`;
      const res = await fetch(url, { method: "POST", credentials: "include" });
      if (!res.ok) throw new Error(await res.text());

      const data = await res.json(); // { userId, isActive }
  ...
  return (
    <div className="al-wrap">
      {loading && <div className="al-alert al-warn">Đang tải danh sách tài khoản...</div>}
      {err && <div className="al-alert al-err">Lỗi: {err}</div>}

      <div className="al-gridCards">
        <StatCard label="Tổng người dùng" value={total} icon={<IconUsers />} tone="amber" />
        <StatCard label="Tài khoản hoạt động" value={activeCount} icon={<IconShield />} tone="emerald" />
        <StatCard label="Tài khoản bị khóa" value={lockedCount} icon={<IconUserX />} tone="red" />
      </div>
      ...
      <div className="al-toolbar">
        ...
        <select className="al-select" value={role} onChange={(e) => setRole(e.target.value)}>
                ...
                users.map((u) => {
                  ...
                  return (
                    <tr key={id}>
                      ...
                      <td>
                        <button
                          className={`al-statusBtn ${isActive ? "al-statusBtnOk" : "al-statusBtnBad"}`}
                          disabled={busy}
                          onClick={() => toggleActive(id, isActive, username || email)}
                          title="Bấm để đổi trạng thái"
                        >
                          {busy ? "Đang cập nhật..." : isActive ? "Đang hoạt động" : "Tạm khóa"}
                        </button>
                      </td>

                      {/* ✅ Thêm button View cạnh button Edit */}
                      <td className="al-tdAction">
                        <button
                          className="al-iconBtn"
                          title="Xem thông tin tài khoản"
                          onClick={() => onViewAccount?.(id)}
                        >
                          <IconEye />
                        </button>

                        <button className="al-iconBtn" title="Sửa">
                          <IconEdit />
                        </button>
                      </td>
                    </tr>
