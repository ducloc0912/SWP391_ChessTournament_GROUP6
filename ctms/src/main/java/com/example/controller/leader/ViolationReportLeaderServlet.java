...
@WebServlet("/api/leader/reports")
public class ViolationReportLeaderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (tid == null || tid.isBlank()) {
            ...
            response.getWriter().write("{\"message\":\"Missing tournamentId\"}");
            return;
        }
        String status = request.getParameter("status");
        try {
            int tournamentId = Integer.parseInt(tid);
            List<ReportDTO> list = reportService.getViolationReportsByTournament(tournamentId, status);
            response.getWriter().write(gson.toJson(list));
        } catch (NumberFormatException e) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write("{\"message\":\"Invalid tournamentId\"}");
        }
    }
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            boolean ok = reportService.decideViolationReport(reportId, valid, note, leader.getUserId());
            if (ok) {
